...
class PhotoStructure:
    ...
    def validate_structure(self) -> list:
        """Validate photo folder structure"""
        issues = []
        
        if not self.base_dir.exists():
            return issues  # No issues if folder doesn't exist
        ...
        for year_dir in self.base_dir.iterdir():
            ...
            if not valid:
                ...
                if year is not None:
                    print(f"  ⚠ Date folder in Photos root: {year_dir.name}")
